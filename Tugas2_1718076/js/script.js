const ApiKey = "342953aecd364afcbd27f36229d7338d";
        ...
        .then(resJson => {
            ...
            resJson.teams.forEach(team => {
                ...
                <li class="collection-item avatar">
                    ...
                    <a class="waves-effect waves-light btn modal-trigger secondary-content" data-id="${team.id}" href="#modal1"><i data-id="${team.id}" class="material-icons">info</i></a>
                </li>
                `
            });
            ...
            detail.forEach(btn => {
                btn.onclick = (event) => {
                    ...
                    getTeamInfo(event.target.dataset.id);
                }
            })
        }).catch(error => {
            console.error(error);
    ...
    .then(response => response.json())
        .then(teamInfo => {
            ...
            `<div class="card-image"  align="center" >
                <img class="responsive-img"  src="${teamInfo.crestUrl}"  ">
             </div>
             <div class="modal-content">
                <h4 align="center">${teamInfo.name}</h4>
                    <table align="center" class="highlight">
                        <tr>
                            <td>Nama pendek</td>
                            <td>${teamInfo.shortName}</td>
                        </tr>
                        <tr>
                            <td>Berdiri</td>
                            <td>${teamInfo.founded}</td>
                        </tr>
                        <tr>
                            <td>Markas</td>
                            <td>${teamInfo.venue}</td>
                        </tr>
                        <tr>
                            <td>Alamat</td>
                            <td>${teamInfo.address}</td>
                        </tr>
                        <tr>
                            <td>No. Telp</td>
